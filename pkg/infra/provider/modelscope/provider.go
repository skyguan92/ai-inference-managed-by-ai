...
func (p *Provider) Pull(ctx context.Context, source, repo, tag string, progressCh chan<- model.PullProgress) (*model.Model, error) {
	if source != "" && source != "modelscope" && source != "ms" {
	...
	m := &model.Model{
		...
		CreatedAt: now,
		UpdatedAt: now,
		Type:      model.ModelType(DetectModelType(info.Data.PipelineTag, info.Data.ModelType)),
	}

	versionID := tag
	if versionID == "" && len(info.Data.Snapshots) > 0 {
		versionID = info.Data.Snapshots[0].VersionID
	}
	...
	for _, file := range info.Data.ModelFileList {
		if file.Type == "Model" || file.Type == "Weights" {
	...
	if len(filesToDownload) == 0 {
		filesToDownload = info.Data.ModelFileList
	...
	for _, file := range filesToDownload {
		...
		fileTotal, err = p.downloadFile(ctx, repo, filename, versionID, destPath, fileProgressCh)
		if err != nil {
	...
	if len(filesToDownload) > 0 {
		mainFile := filesToDownload[0].Name
		m.Format = model.ModelFormat(GetModelFormat(mainFile))
		if m.Format == "" {
			m.Format = model.FormatGGUF
		}
	}
	...
	if len(filesToDownload) > 0 && filesToDownload[0].Size > 0 {
		m.Checksum = fmt.Sprintf("size:%d", filesToDownload[0].Size)
	}
...
func (p *Provider) downloadFile(ctx context.Context, repo, filename, versionID, destPath string, progressCh chan<- int64) (int64, error) {
	reader, _, err := p.client.DownloadFile(ctx, repo, filename, versionID, func(downloaded, total int64) {
		if progressCh != nil {
...
func (p *Provider) Search(ctx context.Context, query string, source string, modelType model.ModelType, limit int) ([]model.ModelSearchResult, error) {
	...
	resp, err := p.client.SearchModels(ctx, query, "", task, limit, 0)
	if err != nil {
		return nil, fmt.Errorf("search models: %w", err)
	}

	if resp.Data == nil {
		return []model.ModelSearchResult{}, nil
	}

	results := make([]model.ModelSearchResult, 0, len(resp.Data.Data))
	for _, item := range resp.Data.Data {
		typ := model.ModelTypeLLM
		if detected := DetectModelType(item.PipelineTag, item.ModelType); detected != "" {
...
func (p *Provider) Verify(ctx context.Context, modelID string, checksum string) (*model.VerificationResult, error) {
	...
	if repo != "" {
		...
		if err != nil {
			issues = append(issues, "repository not accessible on ModelScope")
		}
	}
...
func (p *Provider) EstimateResources(ctx context.Context, modelID string) (*model.ModelRequirements, error) {
	...
	var totalSize int64
	if info.Data.FileSummary != nil {
		totalSize = info.Data.FileSummary.TotalSize
	}
	...
	} else if info.Data.Gigabytes > 0 {
		memMin = int64(info.Data.Gigabytes * 1024 * 1024 * 1024 * 1.2)
		memRec = int64(info.Data.Gigabytes * 1024 * 1024 * 1024 * 1.5)
	} else {
