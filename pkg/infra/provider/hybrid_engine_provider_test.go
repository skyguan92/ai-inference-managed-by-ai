...
func TestHybridEngineProvider_getDockerImages(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			engineName:    "vllm",
			version:       "",
			wantMinImages: 2, // YAML: main image + 1 alternative
			wantContains:  "vllm",
		},
		{
			name:          "whisper images",
...
func TestHybridEngineProvider_buildDockerCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "vllm default image uses YAML command",
			engineType:  "vllm",
		...
		{
			name:        "vllm with config still uses YAML defaults",
			engineType:  "vllm",
			image:       "vllm/vllm-openai:v0.15.0",
		...
		{
			name:        "asr uses YAML uvicorn command",
			engineType:  "asr",
		...
		{
			name:        "whisper falls back to hardcoded (no YAML)",
			engineType:  "whisper",
		...
		{
			name:        "tts other image still uses YAML",
			engineType:  "tts",
